...
from captcha_trainer.config import *
from captcha_trainer.constants import RunMode
from captcha_trainer.encoder import Encoder
from captcha_trainer.core import NeuralNetwork
...
class Predict:
    ...
    def testing(self, image_dir, limit=None):
        ...
        for i, p in enumerate(dir_list):
            ...
            #     f.write("\n".join(lines))
        sess.close()
    def getstr(self, path,flie_name, limit=None):
        lst=[]
        image_dir=path+'/'+flie_name
        graph = tf.Graph()
        sess = tf.Session(
            graph=graph,
            config=tf.ConfigProto(
        ...
        et = time.time()
        label = re.search(self.model_conf.extract_regex, flie_name.split(PATH_SPLIT)[-1])
        label = label.group() if label else flie_name.split(".")[0]
            # if is_mark:
        if 'LOWER' in self.model_conf.category_param:
            label = label.lower()
            t = label == predict_text.lower()
        ...
        if t:
            ...
            false_count += 1
        print(label, predict_text, t, true_count / (true_count + false_count), (et-st) * 1000)
        lst.append(predict_text)
        sess.close()
        return lst

def predict_img(modelname,path,fliename):
    predict=Predict(modelname)
    s=predict.getstr(path,fliename, limit=0)
    return s
if __name__ == '__main__':
    # predict = Predict(project_name=sys.argv[1])
    # predict.testing(image_dir=r"H:\TrainSet\*", limit=None)
    predict=Predict('360-CNNX-GRU-H64-CTC-C1')
    predict.testing(image_dir="E:/codeworker/CTF/360img/", limit=None)
